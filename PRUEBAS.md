...
**Usuario de prueba (seed):** `admin@streamzone.com` / `admin123` (id de usuario: 1).
...
| P-04 | Login correcto | 1. En login, email `admin@streamzone.com` y contraseña `admin123`.<br>2. Pulsar **Entrar**. | Redirección a `/home`; sesión guardada (`id`, `username`, `email` en localStorage). | Acceso a Home con nombre de usuario visible. | ✅ Correcto |
| P-05 | Login incorrecto | 1. Email `admin@streamzone.com` y contraseña errónea.<br>2. Pulsar **Entrar**. | Mensaje de error; botón deja de cargar; no se accede a Home. | Mensaje *"Email o contraseña incorrectos"*; loading se desactiva. | ✅ Correcto |
...
- **Registro:** endpoint `POST /api/users/register` disponible en backend; no integrado en UI de login.
...
| Prioridad | Mejora |
|-----------|--------|
| Alta | Hash de contraseñas con **bcrypt** y autenticación **JWT**. |
| Alta | Guards en backend que validen el usuario autenticado en cada ruta protegida. |
| Media | Pantalla de registro en Angular conectada a `POST /api/users/register`. |
...
3. P-14 (comprobar proxy en pestaña Red del navegador).
4. P-06 → P-09 (flujo favoritos TMDB).
5. P-10 → P-12 (flujo ver más tarde TMDB).
6. P-13 (validación en PostgreSQL).
7. Revisar integraciones I-01 a I-06 como checklist transversal.
